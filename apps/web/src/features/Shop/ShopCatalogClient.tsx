...
const ProductCard = ({ locale, product }: { locale: Locale; product: Product }) => {
  ...
  return (
    <Card className="group/card relative flex flex-col" href={`/shop/${product.slug}`}>
      <div className={`
        relative aspect-4/5 w-full overflow-hidden rounded-lg bg-surface-tint/50
      `}>
        <FallbackImage
          alt={product.name}
          className={clsx("object-cover transition-opacity duration-500", secondImage && `
        ...
        {totalStock === 0 && (
          <div className={`
            absolute inset-0 flex items-center justify-center bg-surface/50
            backdrop-blur-sm
          `}>
            <span className={`
