...
describe("calculate - buying", () => {
  ...
  it("equity uses the property value and mortgage balance at the end of year 7", () => {
    const futureValue = BASE.propertyValue * Math.pow(1 + BASE.propertyAppreciation / 100, BASE.years);
    ...
    expect(r2(result.equity)).toBe(r2(futureValue - balance));
  });

  it("selling fees apply to the property value at the end of year 7", () => {
    const futureValue = BASE.propertyValue * Math.pow(1 + BASE.propertyAppreciation / 100, BASE.years);
    expect(r2(result.sellingFees)).toBe(r2(futureValue * (BASE.saleFeesPct / 100)));
  });
  ...
  it("buying net reconciles to every displayed buying row", () => {
    expect(r2(result.buyingNet)).toBe(
      r2(
        -result.deposit -
          result.stampDuty -
          result.totalMortgagePayments +
          result.equity -
          result.initialBuyingCosts -
          result.maintenance -
          result.totalInsurance -
          result.sellingFees -
          result.mortgageArrangementFee -
          result.totalRemortgagingCosts -
          result.totalServiceCharges -
          result.totalGroundRent,
      ),
    );
  });
...
describe("calculate - renting", () => {
  ...
  it("initial savings include all one-off buying costs (£71,000)", () => {
    expect(result.initialSavings).toBe(71_000);
  });

  it("return on initial savings includes the mortgage arrangement fee", () => {
    expect(r2(result.returnOnInitialSavings)).toBe(r2(71_000 * (Math.pow(1.025, 7) - 1)));
  });
...
describe("calculate - summary", () => {
  it("buying is better after 7 years for the base scenario", () => {
...
describe("buildChartData", () => {
  ...
  it("buying at year 7 matches the calculation summary", () => {
    expect(data[6].buying).toBe(Math.round(-calculate(BASE).buyingNet));
  });

  it("renting at year 7 matches the calculation summary", () => {
    expect(data[6].renting).toBe(Math.round(-calculate(BASE).rentingNet));
  });
