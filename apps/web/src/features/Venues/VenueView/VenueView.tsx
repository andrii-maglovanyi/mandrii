                    ...
                    <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3`}>
                      {ongoingEvents.map((event) => (
                        <div className="relative" key={event.id as string}>
                          <div
                            className={`border-primary/30 bg-primary/10 absolute -top-2 -right-2 z-10 rounded-full border-2 px-3 py-1`}
                          >
                            <span className="text-primary text-xs font-bold">{i18n("LIVE")}</span>
                          </div>
