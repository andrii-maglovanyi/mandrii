    ...
    <div className="flex flex-col pt-0.5 pb-2" id={String(venue.id)} key={venue.id.toString()}>
      ...
      >
        <div className="flex w-full flex-col p-4">
          <CardHeader venue={venue} hideUntilHover />
          ...
          {description && (
            <RichText as="p" className="text-neutral line-clamp-3 text-sm">
              {description.replaceAll("\n", "<br />")}
            </RichText>
          )}

          <CardMetadata variant="list" venue={venue} hideUntilHover />
          <CardFooter venue={venue} hideUntilHover />
        </div>
      </section>
    </div>
  );
