  ...
  return (
    <div className="mx-auto max-w-5xl min-w-0" data-offline-article>
      <article className={`prose dark:prose-invert max-w-none space-y-6`}>
        <h1
          className={`mb-8 bg-[linear-gradient(to_right,var(--color-neutral)_0%,var(--color-on-surface)_30%,var(--color-on-surface)_70%,var(--color-neutral)_100%)] bg-clip-text text-center text-4xl wrap-break-word text-transparent sm:text-5xl md:text-7xl`}
        >
          {meta.title}
        </h1>
        {meta.description ? <p className={`text-neutral mb-8 text-center text-2xl`}>{meta.description}</p> : null}
        {meta.images?.length ? (
          <div className={`relative h-96 w-full overflow-hidden rounded-lg sm:h-120 md:h-160 lg:h-180`}>
            <ImageCarousel
