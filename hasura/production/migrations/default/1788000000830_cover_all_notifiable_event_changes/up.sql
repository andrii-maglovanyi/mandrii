-- Event followers should hear about any public-facing change, including a
-- venue/type/contact change. The existing updated_at trigger runs before this
-- AFTER trigger, so source_at remains the exact mutation time.
...
BEGIN
  ...
  ELSIF OLD.status = 'ACTIVE' AND (
    ...
    AND (
      ...
      OR NEW.organizer_phone_number IS DISTINCT FROM OLD.organizer_phone_number
